...
praesi %>%
  group_by(gender) %>%
  summarise(Aneignungsverhalten_mean = mean(Aneignungsverhalten, na.rm = TRUE)-1, 
            count = n(),
...
mutate(Aneignungsverhalten_ci = Aneignungsverhalten_se * 1.96) %>%
  ...
  labs(title = "Divers und Frauen haben ein geringeres Aneignungsverhalten als Männer", 
...
cor(df$proTrust, df$Aneignungsverhalten, method = "pearson")

##H01: Pearson (sozTrust mit Aneignungsverhalten)
df <- data.rds

cor(df$sozTrust, df$Aneignungsverhalten, method = "pearson")
